...
pub struct Log {
    ...
    // Cache for log writer.
    last_epoch_directory: Option<Box<(PathBuf, time::PrimitiveDateTime)>>,
...
impl Log {
    ...
    pub fn prune_cache(&mut self, prune_before: time::PrimitiveDateTime) {
        ...
        while let Some((key, _pile)) = self.piles.first_key_value() {
            if *key >= prune_before {
                break;
            }
...
fn test_log_lifetime() {
    ...
    let mut writer1 = log.get_writer();
    let maintainer = log.clone().into_maintainer();
    let mut reader = log.into_reader();
    let mut writer2 = Log::new(Arc::new(temp.path("test_log_lifetime"))).into_writer();
