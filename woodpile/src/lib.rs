//! The `woodpile` crate implement a protocol for loosely
//! time-synchronised bags of records in a way that should play nicely
//! with NFS.  Each woodpile "topic" is simply a directory tree.
//!
//! The time-synchronised bags do not implement any form of mutual
//! exclusion or conditional stores; `woodpile` storage is instead
//! expected to be combined with a lease mechanism for mutual
//! exclusion in the happy path.  When something does go wrong, leases
//! are expected to be held on for long enough (a lease should expire
//! at least 20 seconds after the timestamp on any record written
//! while holding the lease) for the loose time synchronisation to
//! kick in and clarify what has and hasn't been committed.
//!
//! Thus, in the common case, `woodpile` minimises coordination in the
//! persistent store.  The downside is that `woodpile` sacrifices
...
pub use log::Log;
pub use log::LogMaintainer;
pub use log::LogReader;
pub use log::LogWriter;
pub use log::ReaderMode;
pub use log::Record;
