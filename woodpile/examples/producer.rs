...
fn main() -> Result<()> {
    ...
    let id = (std::process::id() as u64) | (rand::random::<u64>() << 32);
    let log_name = format!("test_{}", id);
    ...
    for i in 0..120_500 {
        ...
        writer
            .write_or_panic(
                &log_name,
                &(i as u128 | ((id as u128) << 64)).to_le_bytes(),
                ...
                |dst| {
                    let ts = if had_to_sleep { now } else { next_write };
