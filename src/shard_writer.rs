...
impl ShardWriter {
    ...
    fn flush_impl(&mut self, min_write: usize) -> Result<()> {
        ...
        self.encoder.iovec().advance_by_bytes(written);
    ...
    pub fn commit_with_false_alarm(
        ...
        let result = (|| {
            while iovec.total_size() > 0 {
                ...
                iovec.advance_by_bytes(written);
            }
...
fn test_shard_writer_simple() {
    ...
    assert_eq!(contents[..2], crate::hcobs::STUFF_SEQUENCE);
    assert_eq!(contents[contents.len() - 2..], crate::hcobs::STUFF_SEQUENCE);

    let mut decoder = crate::hcobs::Decoder::new();
    decoder
...
fn test_shard_writer_late_late() {
    ...
    let past_deadline = crate::VouchedTime::new(
